...
export type CoverageStatus = "validated" | "draft" | "unvalidated";
export type ItemSource = "text" | "photo";
...
export interface CoverageMap {
  chapters: Readonly<Record<string, CoverageStatus>>;
 ...
 */
export function makeItemDescription(text: string, source: ItemSource) {
 ...
 */
export function makeClassificationResult(raw: UnknownRecord) {
 ...
 */
export function makeClassificationDecision(raw: UnknownRecord) {
  if (!raw || typeof raw !== "object") {
...
export function makeCoverageMap(
  candidateChapters: readonly string[],
  loadedChapterNumbers: readonly string[],
): CoverageMap {
  const loadedSet = new Set(loadedChapterNumbers);
  const chapters: Record<string, CoverageStatus> = {};
