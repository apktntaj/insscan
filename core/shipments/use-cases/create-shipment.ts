 ...
 * @returns {number}
 */
function getNextSerial(existingShipments: readonly Shipment[], prefix: string): number {
 ...
 */
export function createCreateShipmentUseCase(repository: ShipmentRepository) {
  /**
   * @param {Object} input - Shipment field values
   * @returns {Promise<{ok: boolean, data?: Object, error?: Object}>}
   */
  async function execute(input: ShipmentInput): Promise<UseCaseResult<Shipment>> {
    ...
    try {
      ...
      return {
        ...
        error: {
          code: "STORAGE_ERROR",
          message: errorMessage(err, "Failed to save shipment record"),
        },
      };
    }
  }
