 ...
 */
export function createTerminateShipmentUseCase(repository: ShipmentRepository) {
  /**
   * @param {number} id
   * @returns {Promise<{ok: boolean, error?: Object}>}
   */
  async function execute(id: number): Promise<UseCaseResult> {
    if (!id && id !== 0) {
    ...
    try {
      ...
      return {
        ...
        error: {
          code: "STORAGE_ERROR",
          message: errorMessage(err, "Failed to terminate shipment record"),
        },
      };
    }
  }
