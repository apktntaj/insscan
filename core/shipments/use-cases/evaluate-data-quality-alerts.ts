...
export type AlertEngine = "data-quality" | "shipment-status";
 ...
 * // => null
 */
export function getHighestRisk(alerts: readonly Alert[]): RiskLevel | null {
...
export function makeAlert(
  ...
  suggestedAction: string,
  engine: AlertEngine,
): AlertFactoryResult<Alert> {
  const knownRuleId = ruleId as keyof typeof ALERT_RULE_IDS;
  // Validate ruleId exists
  if (!ALERT_RULE_IDS[knownRuleId]) {
    return { ok: false, error: `ruleId '${ruleId}' tidak dikenal` };
  }

  // Validate ruleId-engine consistency
  const expectedEngine = RULE_ENGINE_MAP[knownRuleId];
 ...
 */
export function makeAlertResult(
  shipmentId: number,
  alerts: Alert[],
): AlertFactoryResult<AlertResult> {
  if (!alerts || alerts.length === 0) {
    return { ok: false, error: 'alerts tidak boleh kosong' };
  }

  const highestRisk = getHighestRisk(alerts) as RiskLevel;
 ...
 * // => []
 */
export function evaluateDataQualityAlerts(
  shipments: readonly AlertableShipment[],
