 ...
 *
 * @param {import('@core/cek-lartas/domain').Requirement} detail
 * @returns {string[]}
 */
 ...
 *
 * @param {{ label: string, details: import('@core/cek-lartas/domain').Requirement[] }} props
 */
 ...
 *
 * @param {{
 *   row: import('@core/cek-lartas/domain').Result,
 *   onCopy: () => Promise<void>,
 *   onExport: () => void
 * }} props
 */
export default function SingleResultCard({ row, onCopy, onExport }) {
    ...
    const borderDetails = requirements.filter((item) => item.category === "Impor Border");
    const postBorderDetails = requirements.filter((item) => item.category === "Impor Post Border");
    const exportDetails = requirements.filter((item) => item.category === "Ekspor Border");
    const hasLartas = requirements.length > 0;
    ...
    return (
        <div className="rounded-3xl border border-sky-100 bg-white p-6 shadow-sm sm:p-7">
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
                <InfoBadge label="Import" active={borderDetails.length > 0 || postBorderDetails.length > 0} />
                <InfoBadge label="Border" active={borderDetails.length > 0} />
                <InfoBadge label="Post Border" active={postBorderDetails.length > 0} />
                <InfoBadge label="Export" active={exportDetails.length > 0} />
            </div>

            {!hasLartas ? (
                <p className="mt-5 rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-600">
                    Status LARTAS terverifikasi: tidak ada persyaratan LARTAS.
                </p>
            ) : (
