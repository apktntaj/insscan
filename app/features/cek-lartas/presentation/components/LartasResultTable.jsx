...
import { formatHsCode, isValidHsCode } from "@core/cek-lartas/domain";
import LartasDocModal from "@/app/features/cek-lartas/presentation/components/cek-lartas/LartasDocModal";
...
function LartasMatrixTable({ rows, viewMode, setViewMode, fileStats }) {
  ...
  const lartasCount = matrixRows.filter((row) => row.hasLartas).length;
  const verifiedCount = matrixRows.filter((row) => row.status === "berhasil").length;
  const failedCount = matrixRows.length - verifiedCount;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-zinc-600">
            ...
            <span className="font-semibold text-zinc-900">{verifiedCount}</span> HS code terverifikasi terkena LARTAS.
            {failedCount > 0 ? ` ${failedCount} pemeriksaan gagal.` : ""}
            ...
            <tbody>
              ...
              {displayedRows.length === 0 ? (
                <tr className="border-t border-zinc-100 text-zinc-600">
                  <td className="px-4 py-4" colSpan={docCodes.length + 3}>
...
function buildMatrixRows(rows) {
  return rows.map((row, idx) => {
    ...
    return {
      ...
      status: row.status,
      reason: row.reason,
      hasLartas: row.status === "berhasil" && details.length > 0,
      detailsByDocCode,
    };
  });
}
...
function extractSectionDetails(row) {
  return row.status === "berhasil" && Array.isArray(row.requirements)
    ? row.requirements
    : [];
}
...
function resultLabel(row) {
  if (row.status === "berhasil") {
    return row.hasLartas ? "LARTAS Ada" : "LARTAS Tidak Ada";
  }
  if (row.reason === "belum-terverifikasi") return "Belum terverifikasi";
  if (row.reason === "tidak-ditemukan") return "Tidak ditemukan";
  if (row.reason === "hscode-tidak-valid") return "HS code tidak valid";
  return "Sumber gagal";
}
 ...
 *
 * @description Displays verified LARTAS results and explicit failures.
